    ...
    extends AbstractDataWriter
{
    ...
    public void writeHeader(final Table table)
    {
        ...
        if (options.isPrintHeaders()) {
            ...
            // Format header into multiple lines and render with dividers and
            // separators.
    ...
    public void writeRow(final RowDetails rowDetails)
    {
        ...
        if (getTextTableExportOptions().getRepeatHeadersAfterRows() > 0) {
            if (rowDetails.getRowIndex() != 0 && rowDetails.getRowIndex() % getTextTableExportOptions()
                            .getRepeatHeadersAfterRows() == 0) {
                printBottomBorder();
                println();
        ...
        printLine(getStyle().getCenterLeftDivider(), getStyle().getCenterLeftRightDivider(), getStyle()
                        .getCenterCenterDivider(), getStyle().getCenterRightLeftDivider(), getStyle()
                                        .getCenterRightDivider(), rowCells);
     ...
     */
    public List<String> align(final CellDetails _cellDetails, final String _cellData, final String _filler)
    {
    ...
    {
        printLine(getStyle().getTopLeftIntersection(), getStyle().getTopRightIntersection(), getStyle()
                        .getTopCenterIntersection(), getStyle().getTopRightLeftIntersection(), getStyle()
                                        .getTopRightIntersection(), getStyle().getTopBorder());
    }

    public void printTopDownBorder()
    {
        printLine(getStyle().getTopDownLeftIntersection(), getStyle().getTopDownRightIntersection(), getStyle()
                        .getTopDownCenterIntersection(), getStyle().getTopDownRightLeftIntersection(), getStyle()
                                        .getTopDownRightIntersection(), getStyle().getTopDownBorder());
    }

    public void printBottomBorder()
    {
        printLine(getStyle().getBottomLeftIntersection(), getStyle().getBottomRightIntersection(), getStyle()
                        .getBottomCenterIntersection(), getStyle().getBottomRightLeftIntersection(), getStyle()
                                        .getBottomRightIntersection(), getStyle().getBottomBorder());
    }
    ...
    public void printLine(final String left, final String leftRight, final String center, final String rightLeft,
                          final String right, final List<List<String>> cells)
    {
    ...
    public void printLine(final String left, final String leftRight, final String center, final String rightLeft,
                          final String right, final String border)
    {
        printLine(left, leftRight, center, rightLeft, right, null, border);
    }

    public void printLine(final String left, final String leftRight, final String center, final String rightLeft,
                          final String right, final List<String> cellValues, String defaultValue)
    {
        ...
        for (int i = 0; i < this.columns.size(); i++) {
            ...
            sb.append(cellValues != null ? cellValues.get(i)
                            : createString(defaultValue, this.columns.get(i).getWidth()));
            ...
            if (i == this.columns.size() - 1) {
                ...
                // If there are two columns, print the left right column and
                // then right column
                sb.append(leftRight);

            } else if (i == this.columns.size() - 2) {
                // If there are three columns, print the left right column and
                // then right column
                sb.append(rightLeft);
    ...
    public List<List<String>> formatHeaderCells()
    {
        ...
        // Init the header lines array which would store the formatted/aligned
        // strings
        final List<List<String>> headerLines = new ArrayList<>();
